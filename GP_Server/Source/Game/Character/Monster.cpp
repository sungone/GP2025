...
void Monster::Init()
{
	...
	FVector newPos{};
	do { newPos = Map::GetInst().GetRandomPos(ZoneType::DEFAULT); } while (GameWorld::GetInst().IsCollisionDetected(newPos));
...
void Monster::Patrol()
{
	static auto& nav = Map::GetInst().GetNavMesh(ZoneType::DEFAULT);
